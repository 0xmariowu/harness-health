...
#   CHECK_DEPS_REQUIRED="node npm git" check-deps.sh
...
usage:
  ...
  CHECK_DEPS_REQUIRED="node npm git" scripts/check-deps.sh

examples:
  scripts/check-deps.sh git node npm       exit 1 if any is missing
  CHECK_DEPS_REQUIRED="docker" scripts/check-deps.sh
EOF
}
...
if [ "$#" -gt 0 ]; then
  DEPS=("$@")
elif [ -n "${CHECK_DEPS_REQUIRED:-}" ]; then
  read -r -a DEPS <<< "$CHECK_DEPS_REQUIRED"
fi

if [ "${#DEPS[@]}" -eq 0 ]; then
  usage
  printf "${YELLOW}warning:${NC} no dependencies specified — pass args or set CHECK_DEPS_REQUIRED\n"
  exit 0
fi
